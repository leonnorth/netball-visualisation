...
function standingsByYear(){
	...
	function updateStreamgraph(){
		...
		//edge case: 2011 has a short season, fill the missing rounds up with the last round for games not played.
		for (var i = 0; i < layers.length; i++){
			while(layers[i].values.length < 14){
				layers[i].values.push(layers[i].values.slice(-1)[0]);
			}
		}
		...
		var sgcount = 0
		streamgraph_1.transition().duration(transDuration)
			.attr("d",function(d){return area(layers[sgcount++].values);});
	...
	function drawFirstStreamgraph(){
		...
		var area = d3.svg.area()
		    .interpolate("step-after")
		    .x(function(d) {return 40 + x(d.date); })
		    .y0(50 + chartHeight/2)
		    .y1(50 + chartHeight/2);
		...
		function drawLeftoverTeamLogos(){
			...
			function drawRectText(){
				leftover_icons.insert("rect", "image")
					...
					.attr("fill",function(d){return teamCols[d.team]})
					.attr("opacity", 0)
					.transition().duration(transDuration*4)
					.attr("opacity", 1.0);
